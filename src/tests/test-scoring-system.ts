...
function calculateClinicalCaseTotal(score: ClinicalCaseScore) {
...
function calculatePracticalSkillsTotal(score: PracticalSkillsScore) {
  return (
...
const testCases = [
  ...
  },
  {
    ...
    expected: 48,
    description: "Все максимальные баллы по всем станциям"
  },
  {
...
function runTests() {
  ...
  testCases.forEach((testCase, index) => {
    ...
    switch (testCase.contest) {
      case 'visit-card':
        actual = calculateVisitCardTotal(testCase.score as VisitCardScore);
        break;
      case 'clinical-case':
        actual = calculateClinicalCaseTotal(testCase.score as ClinicalCaseScore);
        break;
      case 'practical-skills':
        actual = calculatePracticalSkillsTotal(testCase.score as PracticalSkillsScore);
        break;
      case 'mind-battle':
      case 'jury-question':
